...
with col1:
    ...
    progress_file = st.file_uploader(
        "Upload academic progress PDF", type="pdf", key="progress")
    if progress_file is not None:
        st.session_state.file = progress_file

with col2:
    st.markdown("**Course Schedule**")
    schedule_file = st.file_uploader(
        "Upload course schedule PDF", type="pdf", key="schedule")
...
if st.button("Generate Academic Advice", type="primary"):
    ...
    if file is None:
        ...
        st.success(f"Read {len(file_bytes)} bytes")
        ...
        with st.spinner("Analyzing documents and generating advice..."):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.text_area(
                        "Generated Email", result['choices'][0]['message']['content'], height=400)
                else:
                    st.error(
